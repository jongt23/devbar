...
function exportarPDFInformePropietario() {
  ...
  const formatoRanking = item => orden === 'facturacion'
    ? `${item.facturacion.toFixed(2)} €`
    : `${formatUnidadesInforme(item.unidades)} uds`;
  ...
  const podio = [topTres[1], topTres[0], topTres[2]].filter(Boolean).map(({ item, puesto }) => `
    <div class="podio-item puesto-${puesto}"><div class="medalla">${iconosPodio[puesto]}</div><div class="podio-nombre">${escapeHtml(item.nombre)}</div><div class="podio-valor">${formatoRanking(item)}</div><div class="podio-base"><strong>${puesto}</strong></div></div>`).join('');
  const barras = topTres.map(({ item, puesto }) => `
    <div class="barra-row"><div class="barra-label"><span>${iconosPodio[puesto]} ${escapeHtml(item.nombre)}</span><strong>${formatoRanking(item)}</strong></div><div class="barra-fondo"><div class="barra barra-${puesto}" style="width:${Math.max(8, (valorRanking(item) / maximoTop) * 100)}%"></div></div></div>`).join('');
