...
import { getDatabase, ref, onValue, get, set, update, push, remove } 
...
function suscribirseAFirebase() {
  ...
  // 4. Escuchar Historial de Ventas
...
function renderVentas() {
  ...
  const tickets = Object.entries(historialData)
    .sort((a,b) => Number(b[1].createdAt || b[1].ts || 0) - Number(a[1].createdAt || a[1].ts || 0));
    
  if (tickets.length === 0) {
    tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:var(--text-dim);padding:30px;">No se registran ventas en el historial de este local.</td></tr>`;
  ...
  tickets.forEach(([tid, t]) => {
    ...
    tr.onclick = () => mostrarDetalleTicketHistorico(tid);
    tr.innerHTML = `
      <td>${fechaTxt}</td>
  ...
  document.getElementById("venta-ticket-medio").textContent = `${(recaudado / conteo).toFixed(2)} €`;
}
...
async function guardarConfigImpresoras() {
  ...
  const getPrinterConfig = (type) => {
    return {
      ...
      paper: "58mm" // Mapeado por defecto
    };
  };
...
async function aplicarFiltrosAuditoria() {
  ...
  listContainer.innerHTML = `
    <div style="display: grid; grid-template-columns: 120px 100px 80px 140px 1fr; gap: 10px; border-bottom: 2px solid var(--border); padding-bottom: 8px; font-weight: 600; font-size: 11px; text-transform: uppercase; color: var(--text-dim);">
      <div>Fecha / Hora</div>
      <div>Camarero</div>
      <div>Mesa</div>
      <div>Acción</div>
  ...
  eventos.forEach(ev => {
    ...
    div.style.gridTemplateColumns = "120px 100px 80px 140px 1fr";
    div.style.gap = "10px";
    ...
    div.innerHTML = `
      <div style="font-family: var(--font-code); color: var(--text-dim);">${dateTxt}<br>${timeTxt}</div>
      <div style="font-weight: 500;">${ev.camarero || "—"}</div>
      <div>Mesa ${ev.mesa || "—"}</div>
      <div style="color: ${info.color}; font-weight: 600;">${info.label}</div>
      <div style="line-height: 1.4;">${ev.detalle || ""}${importeStr}</div>
    `;
